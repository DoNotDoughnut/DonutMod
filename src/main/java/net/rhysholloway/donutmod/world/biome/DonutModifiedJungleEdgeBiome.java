...
public final class DonutModifiedJungleEdgeBiome extends Biome {
	
	   public DonutModifiedJungleEdgeBiome() {
		   super(new Biome.Settings().configureSurfaceBuilder(SurfaceBuilderList.JUNGLE, SurfaceBuilderList.MUD_CONFIG).precipitation(Biome.Precipitation.RAIN).category(Biome.Category.JUNGLE).depth(0.2F).scale(0.4F).temperature(0.95F).downfall(0.8F).effects((new BiomeEffects.Builder()).waterColor(4159204).waterFogColor(329011).fogColor(12638463).moodSound(BiomeMoodSound.CAVE).build()).parent("jungle_edge"));
